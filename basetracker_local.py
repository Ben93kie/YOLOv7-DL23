...
class BaseTracker(ABC):
    ...
    def plot_box_on_img(
            ...
            cls: int,
            id: int,
            dist: Optional[float] = None    # ← compatible with Py 3.8
    ) -> np.ndarray:
        ...
        if dist is None:
            label = f'id:{id}  conf:{conf:.2f}  c:{cls}'
        else:
            label = f'id:{id}  d:{dist:.1f} m  conf:{conf:.2f}  c:{cls}'
                ...
                for a in active_tracks:
                    if a.history_observations:
                        if len(a.history_observations) > 2:
                            ...
                            img = self.plot_box_on_img(img, box, a.conf, a.cls, a.id, a.avg_dist)
